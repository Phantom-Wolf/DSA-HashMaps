class HashMap {
	...
	set(key, value) {
		...
		if (loadRatio > this.MAX_LOAD_RATIO) {
			// Minimize the chance a value ends up a long way from its hash position
		...
		if (this._hashTable[index] === undefined) {
			...
			// Separate chaining to resolve hash table collisions
	...
	delete(key) {
		// Open addressing adds complexity here, because we can't rely on the hashed index to be accurate every time.
	...
	_findSlot(key) {
		// O(1) assuming we are avoiding excessive collisions
	...
	_resize(size) {
		// O(n) because were are looping over each key/value in order to add them to our new, larger hash table
